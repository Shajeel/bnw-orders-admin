...
const VendorsPage = () => {
  ...
  const fetchVendors = async () => {
    try {
      ...
      console.log('[Vendors] Fetching with params:', params);
  ...
  const handleSubmit = async (data: VendorFormData) => {
    try {
      ...
      if (selectedVendor) {
        await vendorService.update(selectedVendor._id, data);
      } else {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: '140px',
      render: (vendor: Vendor) => (
        <span className="text-gray-700">{vendor.city}</span>
      ),
    },
    {
      header: 'Status',
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage vendor relationships</p>
          </div>
          <Button
            variant="primary"
          ...
          {isLoading ? (
            ...
            <>
              <Table
                columns={columns}
                data={vendors}
                onRowClick={(vendor) => router.push(`/vendors/${vendor._id}`)}
